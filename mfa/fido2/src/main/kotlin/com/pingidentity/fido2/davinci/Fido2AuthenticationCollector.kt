 ...
 *
 * - **Fido2AuthenticationClientCollector**: Uses Google Play Services FIDO2 API
 *   for device-bound credentials. Provides broader device compatibility.
 *
 * - **Fido2AuthenticationCredentialCollector**: Uses Android Credential Manager API
 *   for discoverable credentials (passkeys). Supports cross-device synchronization.
 *
 * The implementation selection is handled automatically by the Fido2Collector
 * based on runtime API availability detection.
 */
interface Fido2AuthenticationCollector {
    /**

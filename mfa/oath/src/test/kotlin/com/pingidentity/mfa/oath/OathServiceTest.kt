...
import com.pingidentity.mfa.commons.storage.CredentialType
...
class OathServiceTest {
    ...
    // Mock objects
    private lateinit var mockStorageClient: MfaStorageClient
    ...
    fun setup() {
        ...
        oathServiceWithCache = OathService(mockStorageClient, configWithCache)
        oathServiceWithoutCache = OathService(mockStorageClient, configWithoutCache)
    ...
    fun `test add credential stores to storage with cache enabled`() {
        // Given
        val credentialSlot = slot<com.pingidentity.mfa.commons.model.MfaOathCredential>()
        
        every {
            mockStorageClient.storeOathCredential(capture(credentialSlot))
        } just runs
        ...
        verify(exactly = 1) { 
            mockStorageClient.storeOathCredential(any()) 
        }
    }
    ...
    fun `test generate code for credential updates HOTP counter when caching enabled`() {
        ...
        every {
            mockStorageClient.storeOathCredential(any<com.pingidentity.mfa.commons.model.MfaOathCredential>())
        } just runs
        ...
        every {
            mockStorageClient.retrieveOathCredential(hotpCredential.id)
        ...
        verify { 
            mockStorageClient.storeOathCredential(match { 
    ...
    fun `test generate code for missing credential throws exception`() {
        ...
        every {
            mockStorageClient.retrieveOathCredential(nonExistentId)
        } returns null
    ...
    fun `test credential is not cached when caching disabled`() {
        ...
        clearMocks(mockStorageClient)
        
        // Setup credential retrieval from storage
        every {
            mockStorageClient.retrieveOathCredential(testCredentialId)

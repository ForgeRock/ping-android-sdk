...
import com.pingidentity.fido.Constants.ACTION_AUTHENTICATE
import com.pingidentity.fido.Constants.ACTION_REGISTER
import com.pingidentity.fido.Constants.FIELD_ACTION
 ...
 * to create a [FidoRegistrationCollector] or [FidoAuthenticationCollector].
 */
open class FidoCollector : Collector<JsonObject>, DaVinciAware {
    ...
    override fun init(input: JsonObject): Collector<JsonObject> {
        ...
        return when (action) {
            ACTION_REGISTER -> FidoRegistrationCollector()
            ACTION_AUTHENTICATE -> FidoAuthenticationCollector()
            else -> throw IllegalArgumentException("$FIELD_ACTION: $action is not supported")
        }.apply {
            this.davinci = this@FidoCollector.davinci
            init(input)
        }
    }

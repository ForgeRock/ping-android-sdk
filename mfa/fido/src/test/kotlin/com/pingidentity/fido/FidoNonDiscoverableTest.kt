...
class FidoNonDiscoverableTest {
    ...
    private lateinit var activityController: ActivityController<FidoNonDiscoverableActivity>
    ...
    fun `Fido2NonDiscoverableActivity should handle missing PendingIntent`()  = runTest{
        ...
        assertFailsWith<IllegalArgumentException> {
            suspendCancellableCoroutine<PublicKeyCredential> {
                // When
                activityController =
                    Robolectric.buildActivity(FidoNonDiscoverableActivity::class.java, intent)
    ...
    fun `Fido2NonDiscoverableActivity should handle successful authentication result`() {
        // Given
        val intent = Intent().apply {
            putExtra(FidoNonDiscoverableActivity.EXTRA_PENDING_INTENT, mockPendingIntent)
        }
        ...
        onActivityResultMethod.invoke(
            activity,
            FidoNonDiscoverableActivity.FIDO_SIGN_IN_REQUEST_CODE,
            Activity.RESULT_OK,
            resultIntent
        )

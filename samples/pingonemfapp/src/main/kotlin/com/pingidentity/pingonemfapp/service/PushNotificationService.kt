package com.pingidentity.pingonemfapp.service
...
import com.pingidentity.pingonemfapp.notification.NotificationActionReceiver
import com.pingidentity.pingonemfapp.notification.NotificationHelper
import com.pingidentity.pingonemfapp.notification.PushNotificationActivity
...
class PushNotificationService : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        scope.launch {
            // Update the device token in the PingOneMFA SDK
            val success = PingOneMFA.register(token)
            diagnosticLogger.d("Device token registration success: $success")
        }
    }
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (remoteMessage.data.isNotEmpty()) {
            ...
            scope.launch {
                // Process the notification via PingOneMFA SDK
                val result = PingOneMFA.collectPush(remoteMessage)
                result.onSuccess {
                    pushNotification -> handleNotification(pushNotification)
                }.onFailure {
                    diagnosticLogger.e("Error processing notification: ${it.message}")
                }
            }
        }
    }

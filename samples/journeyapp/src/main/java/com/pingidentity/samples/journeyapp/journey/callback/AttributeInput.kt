...
import com.pingidentity.journey.callback.FailedPolicy
...
val VALIDATION_ERROR = mutableMapOf(
    ...
    "LENGTH_BASED" to "{prompt} Must be between {min-password-length} and {max-password-length} characters long",
)
...
fun AbstractValidatedCallback.error(prompt: String): String {
    return failedPolicies.joinToString("\n") { policy ->
        VALIDATION_ERROR[policy.policyRequirement]?.let { error ->
            policy.format(prompt, error)
        } ?: ""
...
fun FailedPolicy.format(prompt: String, message: String): String {
    var msg = message.replace("{prompt}", prompt)

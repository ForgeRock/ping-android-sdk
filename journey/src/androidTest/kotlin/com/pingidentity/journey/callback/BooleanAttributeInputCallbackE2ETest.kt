...
class BooleanAttributeInputCallbackE2ETest : BaseJourneyTest() {
    ...
    fun booleanAttributeInputCallbackTest() = runTest {
        ...
        val callback = node.callbacks.first() as BooleanAttributeInputCallback

        // Assert that the callback properties are set correctly
        assertEquals("preferences/marketing", callback.name)
        assertEquals("Send me special offers and services", callback.prompt)
        assertEquals(true, callback.required)
        assertEquals("{}", callback.policies.toString())
        assertTrue(callback.failedPolicies.isEmpty())
        assertEquals(false, callback.validateOnly)
        assertEquals(false, callback.value)  // Default value is false

        // Set the value to true and continue
        callback.value = true

...
when (node) {
    ...
    is ErrorNode -> {
        val errorMessage = node.message // Retrieve the server-provided error message
        val rawResponse = node.input // Access the raw JSON error response
        // Display the specific error message to the user
    }
    is FailureNode -> {
        val errorCause = node.cause // Retrieve the underlying error
...
For applications using Jetpack Compose, you can seamlessly integrate the Journey SDK using a
`ViewModel` to manage the
UI state.
...
class AuthViewModel : ViewModel() {
    ...
    fun startJourney(flowName: String = "login") {
...
```kotlin
// Retrieve the existing user session. If an active session cookie (ST) exists, 'user' will not be null.
// Note: Even if a user object is retrieved, the access and refresh tokens within might be expired.
val user: User? = journey.user()
val session: SSOToken? = user?.ssoToken() // Retrieve the SSO token if available

user?.let {
    val accessToken = it.accessToken() // Retrieve the current access token
    it.revoke() // Revoke the current access and refresh tokens
    it.userinfo() // Fetch user information using the access token (if valid)
    it.logout() // Initiate the logout process, potentially clearing local session data
}
```
...
C4Context
    ...
    Component(android, "Android", "Library", "Android Specific Functionality")
    ...
    Rel(journey, android, "Depends on")
    Rel(journey, utils, "Depends on")

```

* [Logger](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/logger)
* [journey-plugin](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/journey-plugin)
* [Oidc](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/oidc)
* [Orchestrate](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/orchestrate)
* [Storage](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/storage)
* [Utils](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/utils)
* [Android](https://github.com/ForgeRock/ping-android-sdk/tree/develop/foundation/android)
